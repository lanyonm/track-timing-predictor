...
def _resolve_racer_name(request: Request, r: str | None) -> str | None:
    """Decode racer name from URL-safe Base64 param, falling back to cookie."""
    if r:
        try:
...
async def get_schedule(request: Request, event_id: int, r: str | None = Query(None)):
    ...
    # URL param wins and updates cookie (FR-009)
    if r and racer_name:
        response.set_cookie(
...
async def set_racer_name(event_id: int = Query(...), name: str = Query("")):
    """Set or clear the racer name cookie and redirect to the schedule."""
    if name.strip():
        ...
        response.set_cookie(
            key="racer_name", value=name.strip(),
            httponly=True, secure=True, samesite="lax", max_age=31536000,
        )
    else:
