...
- What happens when a shared URL contains a corrupted or invalid encoded racer name?
  - The system returns no racer name (unhighlighted schedule) rather than silently falling back to the viewer's own cookie. An explicitly provided URL parameter that fails decoding must not serve stale data from a different context. This is logged at WARNING level for operational visibility.
...
- **FR-002**: System MUST provide a name input on the schedule view page in a new `.racer-form-bar` row between the existing meta bar and the schedule container. The form is a flex row containing: a text input with placeholder "Your name to highlight your events" (pre-filled when name is set), a "Highlight" submit button, and a "Clear" text link (visible only when a name is active). On mobile, the form stacks full-width. A `<small>` hint below the input reads "Enter your full name as shown on the start list", visible only when no racer name is active (hidden after a name is submitted to reduce visual clutter). The text input MUST include `aria-label="Racer name"` and `aria-describedby` referencing the hint element, ensuring the form is accessible to screen reader users.
...
- **FR-008**: System MUST include the racer name in the page URL in an encoded (non-plaintext) format upon form submission (not live during typing) so personalized schedules can be bookmarked and shared without exposing the name as readable text. If the URL `r` parameter is present but fails Base64 decoding, the system MUST return no racer name rather than falling back to the cookie — an explicitly provided URL parameter takes precedence, even when invalid.
