...
- **FR-012**: The import script MUST decompose event names into structured category dimensions (discipline, category, gender, age group, round, and modifiers) rather than mapping to a flat discipline key. "Category" is a racing classification level (A/B/C/D) mapping to skill/experience tiers (e.g., Open A = Elite, Open B = Cat 3, Masters A = youngest masters bracket, Masters B/C/D = older age brackets). This compositional approach MUST handle the combinatorial variety of event naming — including category designations, age brackets, masters categories, para classifications, compound groups (e.g., "Elite/Junior"), omnium parts, and round/ride designators.
...
- **FR-014**: The learning database schema MUST be extended to store and query duration observations by structured category dimensions (discipline, category, gender, age group) so that the app can return granular learned averages rather than aggregating all observations for a discipline into a single average. Queries MUST use a cascading fallback: (discipline, category, gender, age_group) → (discipline, category, gender) → (discipline, gender) → (discipline) → static default, returning the most specific level that has sufficient samples.
...
- **Duration Record**: A single observation of how long an event took. Contains structured category information (discipline, category, gender, age group, round, and modifiers like ride number or omnium part), duration in minutes, duration source indicator (how the duration was determined), heat count (optional — present when determinable from start-list or result pages), source identifier, competition ID, session ID, event position, original event name, and a timestamp of when the observation was recorded. When both duration and heat count are present, the per-heat duration (duration ÷ heat count, minus changeover) can be computed to improve per-heat estimates. The structured category enables granular analysis — e.g., comparing duration patterns across age groups or genders within the same discipline.
- **Event Category**: The structured decomposition of an event name into its component dimensions: discipline (the type of race), category (racing classification level — A/B/C/D — mapping to skill/experience tiers such as Elite, Cat 3, Cat 4, or masters age brackets), gender, age group, round (qualifying, final, repechage, etc.), ride number (for multi-ride rounds), and omnium part number (for multi-discipline omnium events). Some dimensions may be absent or unresolvable for a given event name.
...
- Q: How should the app fall back across category granularity when querying learned durations? → A: Cascading fallback: (discipline, category, gender, age_group) → (discipline, category, gender) → (discipline, gender) → (discipline) → static default. "Category" is a racing classification level (A/B/C/D) mapping to skill/experience tiers (e.g., Open A = Elite, Open B = Cat 3, Masters A = youngest masters bracket, Masters B/C/D = older brackets). It is a distinct dimension from age_group.
