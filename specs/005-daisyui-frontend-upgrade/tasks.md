...
- [x] T004 [P] [US1] Convert `app/templates/defaults.html` to DaisyUI: wrap table in `card bg-base-100 shadow-md` with `overflow-x-auto`, apply `table table-sm`, back link with `link link-hover`. Reference: `docs/daisyui-defaults-prototype.html`

- [x] T005 [P] [US1] Convert `app/templates/learned.html` to DaisyUI: same card+table pattern as defaults, remove colored badges from "Used?" column — use plain text with `opacity-50` for "Not yet" rows, add styled empty state with icon and message. Reference: `docs/daisyui-learned-prototype.html`
